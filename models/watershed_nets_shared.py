...
##FC
# Input -64 -64 out
# Value Input 64-64-out

# LSTMFC
#Input -64 -64 LSTM(64) - out
#Value Input -64 -64 LSTM(64) - out
...
class LSTMFCNet(RecurrentTFModelV2):
    ...
    def __init__(self,
        ...
        try:
            ...
            self.causal = False
        # Define input layers
        common_input = tf.keras.layers.Input(
            shape=(None, 4), name="inputs")
        ...
        if share_comm_layer:
            ...
            new_layer = tf.keras.layers.Dense(
                ...
                activation=tf.nn.relu,
                kernel_initializer=normc_initializer(1.0))
        dense1 = new_layer(common_input)
        concat = tf.concat([dense1, additional_input], axis=-1)

        dense2 = tf.keras.layers.Dense(
            32, activation=tf.nn.relu, name="dense2")(concat)
        ...
        self.rnn_model = tf.keras.Model(
            inputs=[common_input, additional_input, seq_in, state_in_h, state_in_c],
    ...
    def forward_rnn(self, inputs, state, seq_lens):
        flowsNreq, commNmyflow = tf.split(inputs, [4, 5], -1)
        model_out, self._value_out, h, c = self.rnn_model([flowsNreq, commNmyflow, seq_lens] +
                                                          state)
...
class CommLSTMFCNet(RecurrentTFModelV2):
    """Example of using the Keras functional API to define a RNN model."""
    ...
    def __init__(self,
                 ...
                 hiddens_size=16,
                 cell_size=128):
        super(CommLSTMFCNet, self).__init__(obs_space, action_space, num_outputs,
                                         model_config, name)
        self.cell_size = cell_size
        try:
            share_comm_layer = model_config["custom_options"]["share_comm_layer"]
        ...
        dense2 = tf.keras.layers.Dense(
            32, activation=tf.nn.relu, name="dense0")(dense1)
        ...
        # Create the RNN model
        self.rnn_model = tf.keras.Model(
            inputs=[common_input, seq_in, state_in_h, state_in_c],
            outputs=[logits, values, state_h, state_c])
        self.register_variables(self.rnn_model.variables)
        print("LSTMFC", self.rnn_model.variables)
